...
void loop() {
 String computerText = Serial.readStringUntil('@');
  ...
  String command = getValue(computerText, ' ',0);
    ...
    if (command == "right" || command == "يمين" || command == "الى يمين") {
      base.write(base_angle -= 20);
    }
    if (command == "left" || command == "يسار" || command == "الى يسار") {
     base.write(base_angle += 20);
    }

    if (command == "top" || command == "اعلى" || command == "الى اعلى") {
      shoulder.write(shoulder_angle -= 20);
    }

   if (command == "bottom"|| command == "اسفل" || command == "الى اسفل") {
     shoulder.write(shoulder_angle += 20);
    }
